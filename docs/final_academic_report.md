...
1. **Phân mảnh ngang nguyên thủy (Primary Horizontal Fragmentation):**
   Bảng `Author` tự phân mảnh dựa trên chữ cái đầu tiên của tên tác giả (Sử dụng phép Chọn - Selection $\sigma$).

   - `Author_1 = σ (name[0] ∈ [A-H]) Author` $\rightarrow$ Cấp phát cho Node A (Port 5001)
   - `Author_2 = σ (name[0] ∈ [I-P]) Author` $\rightarrow$ Cấp phát cho Node B (Port 5002)
   - `Author_3 = σ (name[0] ∈ [Q-Z]) Author` $\rightarrow$ Cấp phát cho Node C (Port 5003)
2. **Phân mảnh ngang dẫn xuất (Derived Horizontal Fragmentation):**
   Bảng `Book` không tự phân mảnh mà bị "dẫn xuất" từ bảng `Author` thông qua khóa ngoại (Sử dụng phép Bán kết - Semi-join $\ltimes$).
